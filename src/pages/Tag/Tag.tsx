...
import {addTag, deleteTag, getTags, updateTag} from '../../apis/tag.ts'
...
export default function Tag() {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      if (!editId) {
        ...
        const res = await updateTag(editId.toString(), text)
        message = res.message
        type = '修改'
      }
      if (message === 'success') {
  ...
  const onDelete = async (recordId: string) => {
    try {
      const {message} = await deleteTag(recordId)
      if (message === 'success') {
